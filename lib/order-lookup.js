 ...
 *     noted its own cart token can ask for exactly one order and get it.
 *  1b. A correlation id the run puts on its own cart, which arrives on the order
 *     as a custom attribute. Exact like the cart token, and the only path that
 *     depends on nothing Shopify might withdraw: the value is ours, and
 *     `Order.customAttributes` is a documented field with no deprecation notice
 *     against it. That the store's own cart attributes survive checkout here was
 *     confirmed on a live order, which arrived carrying admin_order_id and
 *     aws_logs_id from the storefront.
 ...
 *
 * `cart_token:` is a real search filter, confirmed against a live store: an unknown
 * key such as `zzz_not_a_filter:` is silently ignored and returns unrelated orders,
 * while `cart_token:` returned exactly the one matching order. From API 2026-07 the
 * `Order` type also exposes `cartToken` as a readable field, so the filter's answer
 * is checked rather than trusted — Shopify has signalled that the field is on its
 * way out, and a filter that quietly stops filtering would otherwise hand back the
 * wrong order in silence.
 *
...
/**
 * The cart attribute a run stamps itself with. The leading underscore is
 * Shopify's convention for an attribute the customer does not see at checkout,
 * which is right for a value that means nothing to them.
 */
const RUN_ATTRIBUTE = '_bb_automation_run';
...
class OrderLookup {
   ...
   */
  async findRunOrder({ correlationId, cartToken, statusUrl, since, productTitles = [] } = {}) {
    const cart = normaliseCartToken(cartToken);
    if (cart) {
      const exact = await this.findOneBy(`cart_token:${cart}`);

      /*
       * The filter's answer is checked against the field rather than trusted.
       * Shopify has signalled that cartToken is on its way out, and a filter that
       * quietly stopped filtering would otherwise hand back the wrong order in
       * silence. Older API versions do not return the field at all, in which case
       * there is nothing to check and the filter stands on its own.
       */
      const returned = normaliseCartToken(exact?.cartToken);
      if (exact && (!returned || returned === cart)) {
        return { ...summarise(exact, this.client.shop), matchedBy: 'cartToken' };
      }
    }
    ...
    // The run's own stamp. Exact, and beholden to no Shopify token surviving
    // checkout, so it is tried before the weaker heuristics below.
    if (correlationId) {
      const stamped = orders.find((order) =>
        (order.customAttributes ?? []).some(
          (attribute) => attribute.key === RUN_ATTRIBUTE && attribute.value === correlationId,
        ),
      );
      if (stamped) return { ...summarise(stamped, this.client.shop), matchedBy: 'correlationId' };
    }
