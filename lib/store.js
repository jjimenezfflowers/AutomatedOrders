...
async function saveOrderConfig(environment, config, client = db()) {
  ...
  const purpose = optionalText(config.purpose);

  return client.$transaction(async (tx) => {
    await tx.orderDraft.upsert({
      where: { environment },
      create: { environment, deliveryDate: config.deliveryDate || null, purpose },
      update: { deliveryDate: config.deliveryDate || null, purpose },
    });
